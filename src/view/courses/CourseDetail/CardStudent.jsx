  ...
  renderCompetencia(competencia, id) {
    ...
    return (
      <tr key={competencia.titulo}>
        <th onClick={() => this.props.setSelected(id)}>{competencia.titulo}</th>
...
class TablaInfoAlumnos extends Component {
  ...
  showEstudentsCompentencia(student) {
    ...
    return (
      <tr key={student.name + student.lastname + "fullnameCompentecias"}>
        <th>{student.name}</th>
        <th>{student.lastname}</th>
        {competencia.subCompetencia.map(ele => (
          <th key={student.name + student.lastname + "Nota" + ele.value}>
  ...
  showEstudentsSubCompetencia(student) {
    ...
    return (
      <tr key={student.name + student.lastname + "fullnameSubCompentecias"}>
        <th>{student.name}</th>
        <th>{student.lastname}</th>
        <th>{this.renderEditing(subCompetencia.value, () => {})}</th>
        <th>{this.renderEditing(subCompetencia.descripcion, () => {})}</th>
      </tr>
    );
  }
  ...
  renderBodyTable() {
    ...
    if (tipo === 1) show = student => this.showEstudentsCompentencia(student);
    if (tipo === 2) show = student => this.showEstudentsSubCompetencia(student);
    return <tbody>{students.map(show)}</tbody>;
  }
  ...
  renderHeadTable() {
    ...
    if (tipo === 2) heads2 = ["Notas", "Descripcion"];
    heads = heads.concat(heads2);
    return (
      <thead>
        <tr>
          {heads.map(ele => (
            <th key={ele + "headas"}> {ele} </th>
          ))}
        </tr>
      </thead>
    );
  ...
  render() {
    return <table>{this.renderHeadTable()}</table>;
  }
}
...
export default class CourseDetail extends Component {
  constructor(props) {
    ...
    let competencias = [
      ...
      },
      {
        ...
        subCompetencia: [
          ...
          {
            title:
              "maneja fuerte de inrmacion para comprender el espacio geografico y el ambiente",
            value: "B",
            descripcion: "cosas de cosas "
          },
          {
    ...
    let students = [
      ...
      {
        name: "estudiante3",
        lastname: "estudiante3",
        competencias: competencias
      },
      {
        name: "estudiante4",
        lastname: "estudiante4",
        competencias: competencias
      },
      {
        name: "estudiante5",
        lastname: "estudiante5",
        competencias: competencias
      },
      {
        name: "estudiante6",
        lastname: "estudiante6",
        competencias: competencias
      }
    ];
  ...
  renderHeadTable() {
    ...
    if (tipo === 1)
      heads = this.state.competencias[selected[0]].subCompetencia.map(
        ele => ele.title
      );

    if (tipo === 2) heads = ["Notas", "Descripcion"];
    return (
      <thead>
        <tr>
          <th>Nombre</th>
    ...
    return (
      <React.Fragment>
        <TablaCompetnecias
          ...
          setSelected={(x, y) => this.setSelected(x, y)}
        ></TablaCompetnecias>
