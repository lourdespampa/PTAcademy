...
(function (root, factory) {
	...
	Blockly.Events.filter = function (a, b) {
		...
		for (var d = [], e = Object.create(null), f = 0, g; g = c[f]; f++) if (!g.isNull()) {
			...
			else if (g.type != Blockly.Events.UI || "click" != g.element || "commentOpen" != l.element && "mutatorOpen" != l.element && "warningOpen" != l.element) e[h] = {
				event: g,
				index: 1
			}, d.push(g)
		}
	...
	Blockly.Events.disableOrphans = function (a) {
		if ((a.type == Blockly.Events.MOVE || a.type == Blockly.Events.CREATE) && a.workspaceId) {
			...
			if (a = b.getBlockById(a.blockId)) {
				...
				if (c && c.isEnabled()) for (b = a.getDescendants(!1), a = 0; c = b[a]; a++) c.setEnabled(!0);
	...
	Blockly.Xml.blockToDom = function (a, b) {
		...
		if (e = a.getNextBlock()) h = Blockly.utils.xml.createElement("next"), h.appendChild(Blockly.Xml.blockToDom(e, b)), c.appendChild(h);
	...
	Blockly.Xml.cloneShadow_ = function (a, b) {
		for (var c = a = a.cloneNode(!0), d; c;) if (b && "shadow" == c.nodeName && c.removeAttribute("id"), c.firstChild) c = c.firstChild;
		else {
		...
		try {
			for (var h = 0; h < e; h++) {
				...
				if ("block" == l || "shadow" == l && !Blockly.Events.recordUndo) {
					...
					if ("comment" == l) b.rendered ? Blockly.WorkspaceCommentSvg ? Blockly.WorkspaceCommentSvg.fromXml(k, b, d) : console.warn("Missing require for Blockly.WorkspaceCommentSvg, ignoring workspace comment.") : Blockly.WorkspaceComment ? Blockly.WorkspaceComment.fromXml(k, b) : console.warn("Missing require for Blockly.WorkspaceComment, ignoring workspace comment.");
					else if ("variables" == l) {
	...
	Blockly.Block.prototype.unplugFromRow_ = function (a) {
		...
		if (b && a && (a = this.getOnlyValueConnection_()) && a.isConnected() && !a.targetBlock().isShadow()) if (a = a.targetConnection, a.disconnect(), a.checkType_(b)) b.connect(a);
	...
	Blockly.Block.prototype.setColour = function (a) {
		...
		if (!isNaN(c) && 0 <= c && 360 >= c) this.hue_ = c, this.colour_ = Blockly.hueToHex(c);
			...
			throw c = 'Invalid colour: "' + b + '"', a != b && (c += ' (from "' + a + '")'), Error(c);
	};
	...
	Blockly.Block.prototype.jsonInitColour_ = function (a, b) {
		if ("colour" in a) if (void 0 === a.colour) console.warn(b + "Undefined colour value.");
		else {
			var c = a.colour;
			try {
				this.setColour(c)
	...
	Blockly.MenuItem.prototype.createDom = function () {
		...
		Blockly.utils.aria.setRole(a, this.roleName_ || (this.checkable_ ? Blockly.utils.aria.Role.MENUITEMCHECKBOX : Blockly.utils.aria.Role.MENUITEM));
	...
	Blockly.BlockSvg.prototype.getRelativeToSurfaceXY = function () {
		...
		if (d) {
			do {
				...
				(d && d != this.workspace.getCanvas() && d != c)
		}
	...
	Blockly.WorkspaceSvg.getTopLevelWorkspaceMetrics_ = function () {
		...
		if (this.toolbox_) if (this.toolboxPosition == Blockly.TOOLBOX_AT_TOP || this.toolboxPosition == Blockly.TOOLBOX_AT_BOTTOM) d.height -= a.height;
		else {
			if (this.toolboxPosition == Blockly.TOOLBOX_AT_LEFT || this.toolboxPosition == Blockly.TOOLBOX_AT_RIGHT) d.width -= a.width
		} else if (this.flyout_) if (this.toolboxPosition == Blockly.TOOLBOX_AT_TOP || this.toolboxPosition == Blockly.TOOLBOX_AT_BOTTOM) d.height -= b.height;
	...
	Blockly.user.keyMap.createSerializedKey = function (a, b) {
		for (var c = "", d = Blockly.utils.object.values(Blockly.user.keyMap.modifierKeys), e = 0, f; f = b[e]; e++) if (-1 < d.indexOf(f)) c += f;
		else
			throw Error(f + " is not a valid modifier key.");
		return c + a
	};
	...
	Blockly.navigation.insertFromFlyout = function () {
		...
		if (b && b.isVisible()) {
			...
			c.isEnabled() ? (b = b.createBlock(c), b.render(), b.setConnectionsHidden(!1), a.getCursor().setCurNode(Blockly.ASTNode.createBlockNode(b)), Blockly.navigation.modify_() || Blockly.navigation.warn_("Something went wrong while inserting a block from the flyout."), Blockly.navigation.focusWorkspace_(), a.getCursor().setCurNode(Blockly.navigation.getTopNode(b)), Blockly.navigation.removeMark_()) : Blockly.navigation.warn_("Can't insert a disabled block.")
	...
	Blockly.onKeyDown_ = function (a) {
		...
		if (!(Blockly.utils.isTargetInput(a) || b.rendered && !b.isVisible())) if (b.options.readOnly) Blockly.navigation.onKeyPress(a);
		else {
			var c = !1;
			if (a.keyCode == Blockly.utils.KeyCodes.ESC) Blockly.hideChaff(), Blockly.navigation.onBlocklyAction(Blockly.navigation.ACTION_EXIT);
				...
				if (a.keyCode == Blockly.utils.KeyCodes.BACKSPACE || a.keyCode == Blockly.utils.KeyCodes.DELETE) {
					...
					a.keyCode == Blockly.utils.KeyCodes.V ? Blockly.clipboardXml_ && (a = Blockly.clipboardSource_, a.isFlyout && (a = a.targetWorkspace), Blockly.clipboardTypeCounts_ && a.isCapacityAvailable(Blockly.clipboardTypeCounts_) && (Blockly.Events.setGroup(!0), a.paste(Blockly.clipboardXml_), Blockly.Events.setGroup(!1))) : a.keyCode == Blockly.utils.KeyCodes.Z && (Blockly.hideChaff(), b.undo(a.shiftKey))
				}
			}
	...
	Blockly.bindEventWithChecks_ = function (a, b, c, d, e, f) {
		var g = !1,
			...
			k = [];
		if (Blockly.utils.global.PointerEvent && b in Blockly.Touch.TOUCH_MAP) for (var l = 0, m; m = Blockly.Touch.TOUCH_MAP[b][l]; l++) a.addEventListener(m, h, !1), k.push([a, m, h]);
	...
	Blockly.bindEvent_ = function (a, b, c, d) {
		var e = function (a) {
			...
			f = [];
		if (Blockly.utils.global.PointerEvent && b in Blockly.Touch.TOUCH_MAP) for (var g = 0, h; h = Blockly.Touch.TOUCH_MAP[b][g]; g++) a.addEventListener(h, e, !1), f.push([a, h, e]);
	...
	Blockly.FieldDropdown.prototype.renderSelectedImage_ = function (a) {
		...
		this.sourceBlock_.RTL ? (a = Blockly.Field.DEFAULT_TEXT_OFFSET - 1, this.imageElement_.setAttribute("x", Blockly.Field.DEFAULT_TEXT_OFFSET + b), this.textElement_.setAttribute("x", a)) : (a = c + b + Blockly.Field.DEFAULT_TEXT_OFFSET + 1, this.textElement_.setAttribute("text-anchor", "end"), this.textElement_.setAttribute("x", a), this.imageElement_.setAttribute("x", Blockly.Field.DEFAULT_TEXT_OFFSET))
	};
	...
	Blockly.CursorSvg.prototype.createCursorSvg_ = function () {
		...
		this.isMarker_ || (Blockly.utils.dom.createSvgElement("animate", {
			...
			values: Blockly.CursorSvg.CURSOR_COLOR + ";transparent;transparent;",
			repeatCount: "indefinite"
		}, this.cursorInput_), Blockly.utils.dom.createSvgElement("animate", {
			attributeType: "XML",
	...
	Blockly.blockRendering.RenderInfo.prototype.alignRowElements_ = function () {
		for (var a = 0, b; b = this.rows[a]; a++) if (b.hasStatement) this.alignStatementRow_(b);
		else {
	...
	Blockly.blockRendering.Drawer.prototype.drawBottom_ = function () {
		...
		for (var c = b.length - 1, d; d = b[c]; c--) Blockly.blockRendering.Types.isNextConnection(d) ? this.outlinePath_ += d.shape.pathRight : Blockly.blockRendering.Types.isLeftSquareCorner(d) ? this.outlinePath_ += Blockly.utils.svgPaths.lineOnAxis("H", a.xPos) : Blockly.blockRendering.Types.isLeftRoundedCorner(d) ? this.outlinePath_ += this.constants_.OUTSIDE_CORNERS.bottomLeft : Blockly.blockRendering.Types.isSpacer(d) && (this.outlinePath_ += Blockly.utils.svgPaths.lineOnAxis("h", -1 * d.width))
	};
	...
	Blockly.Toolbox.prototype.handleAfterTreeSelected_ = function (a, b) {
		...
		if (a != b && a != this) {
			var c = new Blockly.Events.Ui(null, "category", a && a.getText(), b && b.getText());
